...
public Action:Command_MDB_ReloadAdmins(client, args) {
  ...
  PrintToConsole(client, "[MitchDB] Reloading the list admins");

  is_reload_running = true;

  new Handle:curl = curl_easy_init();
  if(curl == INVALID_HANDLE) {
    LogToGame("[MitchDB] ERROR: Unable to create cURL resource. (admin list)");
    return Plugin_Handled;
  }
...
public onCompleteMDBAdminList(Handle:hndl, CURLcode: code, any:clientid) {
  ...
  if(code != CURLE_OK) {
    if(code == CURLE_COULDNT_RESOLVE_HOST) {
      LogToGame("[MitchDB] ERROR: Network error contacting API. [unable to resolve host] (admin list)");
    } else if(code==CURLE_OPERATION_TIMEDOUT) {
      LogToGame("[MitchDB] ERROR: Network error contacting API. [timed out] (admin list)");
    } else {
      LogToGame("[MitchDB] ERROR: Network error contacting API. [curlcode=%d] (admin list)", code);
    }
    CloseHandle(hndl);
    return;
  }
